...
// ---- Generic unwrapping (handles many API shapes) ----
...
const statusColor = (displayStatus: string) => {
  switch (displayStatus) {
    ...
    case 'Partially Received': return '#5C8CB6';
...
const columnLabel = (k: keyof ColumnVis, role: User['role']) => {
  const map: Record<keyof ColumnVis, string> = {
    ...
    stillToBeDelivered: 'Still to be Delivered (qty)',
    plant: 'Plant',
...
export function PurchaseOrder({ user }: PurchaseOrderProps) {
  ...
  const toggleColumn = (column: keyof ColumnVis) =>
  ...
  const fetchPurchaseOrders = useCallback(async (tab: StatusTab) => {
    setLoading(true);
    setLoadError(null);
    ...
    try {
      ...
      if (tab !== 'all') {
        url.searchParams.set('status', STATUS_TAB_TO_PARAM[tab as Exclude<StatusTab, 'all'>]);
      }
      ...
      const { summary, items } = unwrapPOPayload(json);

      setSummary(summary ?? null);
      setOrders(Array.isArray(items) ? items : []);
      setCurrentPage(1);
      console.debug('[PO] loaded rows:', Array.isArray(items) ? items.length : 0);
    } catch (err: any) {
  ...
  // ----- Dynamic filter options (based on scoped data) -----
  ...
  const filteredOrders = useMemo(() => {
    ...
    scopedOrders, searchQuery, filterStatus, filterStorage,
    filterPlant, filterGroup, filterSupplier, filterDocType
  ]);
  ...
  const ordersNeedingUpdate = useMemo(() => {
    ...
    return filteredOrders.filter(o => {
      if (mapBackendStatusToDisplay(o.status) !== 'On Delivery') return false;
      if (!o.reEtaDate) return false;
      const diffDays = Math.ceil((new Date(o.reEtaDate).getTime() - Date.now()) / 86400000);
  ...
  const handleItemsPerPageChange = (v: string) => { setItemsPerPage(Number(v)); setCurrentPage(1); };

  const handleOpenUpdateDialog = (order: PurchaseOrderItem) => {
  ...
  // ---- Upload helpers (Excel-only validation) ----
  const isExcelFile = (file: File): boolean => {
    const name = file.name.toLowerCase();
    return ALLOWED_EXCEL_EXTENSIONS.some(ext => name.endsWith(ext));
  };

  const processFile = (file: File | null, input?: HTMLInputElement | null) => {
    if (!file) {
      setUploadFile(null);
  ...
  const handleSubmitUploadPO = async () => {
    if (!uploadFile) {
    ...
    try {
      ...
      // backend expects "file" (request.files.get("file"))
  ...
  const handleDownloadTemplate = () => {
    try {
      // Prepare headers for each sheet
      const me2nHeaders = [
      ...
      // Create sheets with headers only (blank template)
      const wb = XLSX.utils.book_new();

      const wsMe2n = XLSX.utils.aoa_to_sheet([me2nHeaders]);
      const wsMe5a = XLSX.utils.aoa_to_sheet([me5aHeaders]);
      const wsZmm = XLSX.utils.aoa_to_sheet([zmm013rHeaders]);

      XLSX.utils.book_append_sheet(wb, wsMe2n, 'ME2N');
      XLSX.utils.book_append_sheet(wb, wsMe5a, 'ME5A');
      XLSX.utils.book_append_sheet(wb, wsZmm, 'ZMM013R');

      const now = new Date();
      const dd = String(now.getDate()).padStart(2, '0');
      const mm = String(now.getMonth() + 1).padStart(2, '0');
      const yyyy = now.getFullYear();
      const filename = `TemplatePO_${dd}-${mm}-${yyyy}.xlsx`;
  ...
  const handleDownloadPOData = () => {
    try {
      // Ordered keys to match table header order
      const orderedKeys: (keyof ColumnVis)[] = [
        ...
        'order', 'changedOn', 'grCreatedDate', 'remarks', 'reEtaDate',
      ];
    ...
    <Card>
      <div className="overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              {visibleColumns.purchaseRequisition && <TableHead>Purchase Requisition</TableHead>}
              {visibleColumns.itemOfRequisition && <TableHead>Item of Requisition</TableHead>}
              {visibleColumns.purchasingDocument && <TableHead>Purchasing Document</TableHead>}
              {visibleColumns.item && <TableHead>Item</TableHead>}
              {visibleColumns.documentDate && <TableHead>Document Date</TableHead>}
              {visibleColumns.deliveryDate && <TableHead>Delivery Date</TableHead>}
              {visibleColumns.purchasingDocType && <TableHead>Purchasing Doc. Type</TableHead>}
              {visibleColumns.purchasingGroup && <TableHead>Purchasing Group</TableHead>}
              {visibleColumns.shortText && <TableHead>Short Text</TableHead>}
              {visibleColumns.material && <TableHead>Material</TableHead>}
              {visibleColumns.nameOfSupplier && <TableHead>Name of Supplier</TableHead>}
              {visibleColumns.quantityReceived && <TableHead>Qty Received</TableHead>}
              {visibleColumns.stillToBeDelivered && <TableHead>Still to be Delivered</TableHead>}
              {visibleColumns.plant && <TableHead>Plant</TableHead>}
              {visibleColumns.storageLocation && <TableHead>Storage Location</TableHead>}
              {visibleColumns.order && <TableHead>Order</TableHead>}
              {visibleColumns.changedOn && (
                <TableHead>{user.role === 'vendor' ? 'Last Updated' : 'Changed On'}</TableHead>
              )}
              {visibleColumns.grCreatedDate && <TableHead>GR Created Date</TableHead>}
              {visibleColumns.remarks && <TableHead>Remarks</TableHead>}
              {visibleColumns.reEtaDate && <TableHead>Re-ETA Date</TableHead>}
              <TableHead className="sticky right-[100px] bg-white shadow-[-2px_0_4px_rgba(0,0,0,0.05)] z-10">Status</TableHead>
              <TableHead className="sticky right-0 bg-white shadow-[-2px_0_4px_rgba(0,0,0,0.05)] z-10">Action</TableHead>
            </TableRow>
          </TableHeader>
            ...
            {pageOrders.map((o, idx) => {
              ...
              return (
                <TableRow key={`${o.purchasingDocument}-${idx}`}>
                  {visibleColumns.purchaseRequisition && <TableCell className="font-medium">{o.purchaseRequisition}</TableCell>}
                  {visibleColumns.itemOfRequisition && <TableCell>{o.itemOfRequisition}</TableCell>}
                  {visibleColumns.purchasingDocument && <TableCell className="font-medium">{o.purchasingDocument}</TableCell>}
                  {visibleColumns.item && <TableCell>{o.item}</TableCell>}
                  {visibleColumns.documentDate && <TableCell className="text-sm text-gray-600">{o.documentDate}</TableCell>}
                  {visibleColumns.deliveryDate && <TableCell className="text-sm text-gray-600">{o.deliveryDate}</TableCell>}
                  {visibleColumns.purchasingDocType && <TableCell>{o.purchasingDocType}</TableCell>}
                  {visibleColumns.purchasingGroup && <TableCell>{o.purchasingGroup}</TableCell>}
                  {visibleColumns.shortText && <TableCell className="max-w-xs truncate">{o.shortText}</TableCell>}
                  {visibleColumns.material && <TableCell className="text-sm text-gray-600">{o.material}</TableCell>}
                  {visibleColumns.nameOfSupplier && <TableCell>{o.nameOfSupplier}</TableCell>}
                  {visibleColumns.quantityReceived && <TableCell className="text-right">{o.quantityReceived}</TableCell>}
                  {visibleColumns.stillToBeDelivered && <TableCell className="text-right">{o.stillToBeDelivered}</TableCell>}
                  {visibleColumns.plant && <TableCell>{o.plant}</TableCell>}
                  {visibleColumns.storageLocation && <TableCell>{o.storageLocation}</TableCell>}
                  {visibleColumns.order && <TableCell className="text-sm text-gray-600">{o.order || '-'}</TableCell>}
                  {visibleColumns.changedOn && <TableCell className="text-sm text-gray-600">{o.changedOn || '-'}</TableCell>}
                  {visibleColumns.grCreatedDate && <TableCell className="text-sm text-gray-600">{o.grCreatedDate || '-'}</TableCell>}
                  {visibleColumns.remarks && <TableCell className="max-w-xs truncate">{o.remarks || '-'}</TableCell>}
                  {visibleColumns.reEtaDate && <TableCell className="text-sm text-gray-600">{o.reEtaDate || '-'}</TableCell>}
  ...
  // ================== Render ==================
  // Root dibuat sama seperti Dashboard: tinggi penuh + satu overflow-y di sini saja
  return (
    <div className="p-4 sm:p-6 lg:p-8 h-full overflow-x-hidden">
      ...
      {!loading && (
        <div className="mb-2 text-xs text-gray-500">
          loaded: {orders.length} • after scope: {scopedOrders.length} • shown: {pageOrders.length}
        </div>
      )}
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                ...
                <div className="space-y-3">
                  ...
                  <ScrollArea className="h-[320px]">
                    <div className="space-y-2 pr-4">
                      ...
                      {/* The rest */}
                      {(Object.keys(visibleColumns) as (keyof ColumnVis)[])
        ...
        {/* Same table for each tab so data always binds */}
